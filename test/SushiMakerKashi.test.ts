...
describe("KashiSushiMaker", function () {
  before(async function () {
    await prepare(this, ["SushiMakerKashi", "SelBar", "SushiMakerKashiExploitMock", "ERC20Mock", "UniswapV2Factory", "UniswapV2Pair", "BentoBoxV1", "KashiPairMediumRiskV1", "PeggedOracleV1"])
  })
  ...
  beforeEach(async function () {
    ...
    await deploy(this, [["kashiMaker", this.SushiMakerKashi, [this.factory.address, this.bar.address, this.bento.address, this.sel.address, this.weth.address, this.factory.pairCodeHash()]]])
    ...
    // Create SLPs
    await createSLP(this, "sushiEth", this.sel, this.weth, getBigNumber(10))
    ...
    await createSLP(this, "sushiUSDC", this.sel, this.usdc, getBigNumber(10))
    ...
    await this.sel.approve(this.bento.address, getBigNumber(10))
    ...
    await this.bento.deposit(this.sel.address, this.alice.address, this.alice.address, getBigNumber(10), 0)
    ...
    //const initData = defaultAbiCoder.encode(["address", "address", "address", "bytes"], [this.sel.address, this.dai.address, this.oracle.address, oracleData])
    //await this.bento.deploy(this.KashiMaster.address, initData, true)
  })

  describe("setBridge", function () {
    it("only allows the owner to set bridge", async function () {
      await expect(this.kashiMaker.connect(this.bob).setBridge(this.sel.address, this.weth.address, { from: this.bob.address })).to.be.revertedWith("Ownable: caller is not the owner")
    })
    
    it("does not allow to set bridge for sel", async function () {
      await expect(this.kashiMaker.setBridge(this.sel.address, this.weth.address)).to.be.revertedWith("Maker: Invalid bridge")
    })

    it("does not allow to set bridge for WETH", async function () {
      await expect(this.kashiMaker.setBridge(this.weth.address, this.sel.address)).to.be.revertedWith("Maker: Invalid bridge")
    })
    ...
    it("emits correct event on bridge", async function () {
      await expect(this.kashiMaker.setBridge(this.dai.address, this.sel.address))
  ...
  describe("convert", function () {
    it("reverts if caller is not EOA", async function () {
      await expect(this.exploiter.convert(this.sel.address)).to.be.revertedWith("Maker: Must use EOA")
    })
  })
})

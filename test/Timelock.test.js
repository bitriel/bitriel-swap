...
contract("Timelock", ([alice, bob, carol, dev, minter]) => {
  ...
  it("should not allow non-owner to do operation", async () => {
    await this.sel.transferOwnership(this.timelock.address, { from: alice });
    await expectRevert(
      this.sel.transferOwnership(carol, { from: alice }),
      "Ownable: caller is not the owner"
    );
    await expectRevert(
      this.sel.transferOwnership(carol, { from: bob }),
      "Ownable: caller is not the owner"
    );
    await expectRevert(
  ...
  it("should do the timelock thing", async () => {
    ...
    assert.equal((await this.sel.owner()).valueOf(), carol);
  });

  it("should also work with SelMaster", async () => {
    ...
    await this.sel.transferOwnership(this.chef.address, { from: alice });

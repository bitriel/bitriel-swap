...
describe("Timelock", function () {
  ...
  it("should not allow non-owner to do operation", async function () {
    await this.sel.transferOwnership(this.timelock.address)
    // await expectRevert(this.sel.transferOwnership(carol, { from: alice }), "Ownable: caller is not the owner")

    await expect(this.sel.transferOwnership(this.carol.address)).to.be.revertedWith("Ownable: caller is not the owner")
    await expect(this.sel.connect(this.bob).transferOwnership(this.carol.address)).to.be.revertedWith("Ownable: caller is not the owner")
  ...
  it("should do the timelock thing", async function () {
    ...
    await this.timelock
      .connect(this.bob)
      .queueTransaction(this.sel.address, "0", "transferOwnership(address)", encodeParameters(["address"], [this.carol.address]), eta)
    await increase(duration.days(1))
    await expect(
      this.timelock
        .connect(this.bob)
        .executeTransaction(this.sel.address, "0", "transferOwnership(address)", encodeParameters(["address"], [this.carol.address]), eta)
    ).to.be.revertedWith("Timelock::executeTransaction: Transaction hasn't surpassed time lock.")
    await increase(duration.days(4))
    await this.timelock
      .connect(this.bob)
      .executeTransaction(this.sel.address, "0", "transferOwnership(address)", encodeParameters(["address"], [this.carol.address]), eta)
    expect(await this.sel.owner()).to.equal(this.carol.address)
  })

  it("should also work with SelMaster", async function () {
    this.lp1 = await this.ERC20Mock.deploy("LPToken", "LP", "10000000000")
    this.lp2 = await this.ERC20Mock.deploy("LPToken", "LP", "10000000000")
    this.chef = await this.SelMaster.deploy(this.sel.address, this.dev.address, "1000", "0", "1000")

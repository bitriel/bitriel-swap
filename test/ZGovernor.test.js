...
contract("GovernorAlpha", ([alice, minter, dev]) => {
  it("should work", async () => {
    this.sel = await SelToken.new({ from: alice });
    await this.sel.delegate(dev, { from: dev });
    this.master = await SelMaster.new(this.sel.address, dev, "100", "0", "0", {
      from: alice,
    });
    ...
    // Perform another deposit to make sure some SELs are minted in that 1 block.
    await this.master.deposit(0, "100", { from: minter });
    assert.equal((await this.sel.totalSupply()).valueOf(), "110");
    assert.equal((await this.sel.balanceOf(minter)).valueOf(), "100");
    assert.equal((await this.sel.balanceOf(dev)).valueOf(), "10");

...
describe("SelMiniV2", function () {
  before(async function () {
    await prepare(this, ['SelMiniV2', 'SelToken', 'ERC20Mock', 'RewarderMock', 'RewarderBrokenMock'])
    await deploy(this, [
  ...
  beforeEach(async function () {
    ...
    await deploy(this,
      ...
      ['chef', this.SelMiniV2, [this.sel.address]],
    ...
    await this.sel.mint(this.chef.address, getBigNumber(10000))
    await this.lp.approve(this.chef.address, getBigNumber(10))
    await this.chef.setSelPerSecond("10000000000000000")
  ...
  describe("PendingSel", function() {
    it("PendingSel should equal ExpectedSel", async function () {
      await this.chef.add(10, this.rlp.address, this.rewarder.address)
      ...
      let expectedsel = BigNumber.from("10000000000000000").mul(timestamp2 - timestamp)
      let pendingSel = await this.chef.pendingSel(0, this.alice.address)
      expect(pendingSel).to.be.equal(expectedsel)
    })
    ...
    it("Should emit event LogUpdatePool", async function () {
      ...
      await expect(this.chef.updatePool(0))
            ...
            .withArgs(0, (await this.chef.poolInfo(0)).lastRewardTime,
              ...
              (await this.chef.poolInfo(0)).accSelPerShare)
    })
  ...
  describe("Harvest", function () {
    it("Should give back the correct amount of sel and reward", async function () {
        ...
        let timestamp = (await ethers.provider.getBlock(log.blockNumber)).timestamp
        let expectedsel = BigNumber.from("10000000000000000").mul(timestamp2 - timestamp)
        expect((await this.chef.userInfo(0, this.alice.address)).rewardDebt).to.be.equal("-"+expectedsel)
        await this.chef.harvest(0, this.alice.address)
        expect(await this.sel.balanceOf(this.alice.address)).to.be.equal(await this.r.balanceOf(this.alice.address)).to.be.equal(expectedsel)
    })
    ...
    it("Harvest for sel-only pool", async function () {
      ...
      expect((await this.chef.userInfo(0, this.alice.address)).rewardDebt).to.be.equal("-"+expectedsel)
      await this.chef.harvest(0, this.alice.address)
      expect(await this.sel.balanceOf(this.alice.address)).to.be.equal(expectedsel)
    })
  })

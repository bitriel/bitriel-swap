...
const SelBar = artifacts.require("SelBar");

contract("SelBar", ([alice, bob, carol]) => {
  beforeEach(async () => {
    this.sel = await SelToken.new({ from: alice });
    this.bar = await SelBar.new(this.sel.address, { from: alice });
    this.sel.mint(alice, "100", { from: alice });
    this.sel.mint(bob, "100", { from: alice });
    this.sel.mint(carol, "100", { from: alice });
  });
  ...
  it("should not allow enter if not enough approve", async () => {
    ...
    await this.sel.approve(this.bar.address, "50", { from: alice });
    await expectRevert(
      this.bar.enter("100", { from: alice }),
      "ERC20: transfer amount exceeds allowance"
    );
    await this.sel.approve(this.bar.address, "100", { from: alice });
  ...
  it("should work with more than one participant", async () => {
    await this.sel.approve(this.bar.address, "100", { from: alice });
    await this.sel.approve(this.bar.address, "100", { from: bob });
    ...
    assert.equal((await this.sel.balanceOf(this.bar.address)).valueOf(), "30");
    // SelBar get 20 more SUSHIs from an external source.
    await this.sel.transfer(this.bar.address, "20", { from: carol });
    ...
    assert.equal((await this.sel.balanceOf(this.bar.address)).valueOf(), "52");
    assert.equal((await this.sel.balanceOf(alice)).valueOf(), "70");
    assert.equal((await this.sel.balanceOf(bob)).valueOf(), "98");
  });
});

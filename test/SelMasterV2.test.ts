...
describe("SelMasterV2", function () {
  before(async function () {
    await prepare(this, ['SelMaster', 'SelToken', 'ERC20Mock', 'SelMasterV2', 'RewarderMock', 'RewarderBrokenMock'])
    await deploy(this, [
  ...
  beforeEach(async function () {
    ...
    await deploy(this,
      ...
      ['chef', this.SelMaster, [this.sel.address, this.bob.address, getBigNumber(100), "0", "0"]]
    ])

    await this.sel.transferOwnership(this.chef.address)
    ...
    await deploy(this, [
        ['chef2', this.SelMasterV2, [this.chef.address, this.sel.address, 1]],
  ...
  describe("PendingSel", function() {
    it("PendingSel should equal ExpectedSel", async function () {
      ...
      await advanceBlock()
      let expectedSel = getBigNumber(100).mul(log2.blockNumber + 1 - log.blockNumber).div(2)
      let pendingSel = await this.chef2.pendingSel(0, this.alice.address)
    ...
    it("When block is lastRewardBlock", async function () {
      ...
      let log2 = await this.chef2.updatePool(0)
      let expectedSel = getBigNumber(100).mul(log2.blockNumber - log.blockNumber).div(2)
      let pendingSel = await this.chef2.pendingSel(0, this.alice.address)
      expect(pendingSel).to.be.equal(expectedSel)
    })
  })
    ...
    it("Should emit event LogUpdatePool", async function () {
      ...
      await expect(this.chef2.updatePool(0))
            ...
            .withArgs(0, (await this.chef2.poolInfo(0)).lastRewardBlock,
              ...
              (await this.chef2.poolInfo(0)).accSelPerShare)
    })
  ...
  describe("Harvest", function () {
    it("Should give back the correct amount of SUSHI and reward", async function () {
        ...
        await this.chef2.harvestFromSelMaster()
        ...
        expect(await this.sel.balanceOf(this.alice.address)).to.be.equal(await this.r.balanceOf(this.alice.address)).to.be.equal(expectedSushi)
    })
    ...
    it("Harvest for SUSHI-only pool", async function () {
      ...
      expect((await this.chef2.userInfo(0, this.alice.address)).rewardDebt).to.be.equal("-"+expectedSushi)
      await this.chef2.harvest(0, this.alice.address)
      expect(await this.sel.balanceOf(this.alice.address)).to.be.equal(expectedSushi)
    })
  })

...
const SelMaster = artifacts.require("SelMaster");
const MockERC20 = artifacts.require("MockERC20");

contract("SelMaster", ([alice, bob, carol, dev, minter]) => {
  beforeEach(async () => {
    this.sel = await SelToken.new({ from: alice });
  });

  it("should set correct state variables", async () => {
    this.master = await SelMaster.new(
      this.sel.address,
      dev,
      "1000",
      "0",
    ...
    await this.sel.transferOwnership(this.master.address, { from: alice });
    const sel = await this.master.sel();
    const devaddr = await this.master.devaddr();
    const owner = await this.sel.owner();
    assert.equal(sel.valueOf(), this.sel.address);
  ...
  context("With ERC/LP token added to the field", () => {
    ...
    it("should give out SELs only after farming time", async () => {
      // 100 per block farming rate starting at block 100 with bonus until block 1000
      this.master = await SelMaster.new(
        this.sel.address,
        dev,
      ...
      assert.equal((await this.sel.balanceOf(bob)).valueOf(), "0");
      ...
      assert.equal((await this.sel.balanceOf(bob)).valueOf(), "1000");
      await time.advanceBlockTo("104");
      await this.master.deposit(0, "0", { from: bob }); // block 105
      assert.equal((await this.sel.balanceOf(bob)).valueOf(), "5000");
      assert.equal((await this.sel.balanceOf(dev)).valueOf(), "500");
      assert.equal((await this.sel.totalSupply()).valueOf(), "5500");
    });
    ...
    it("should not distribute SUSHIs if no one deposit", async () => {
      ...
      assert.equal((await this.sel.totalSupply()).valueOf(), "0");
      ...
      assert.equal((await this.sel.totalSupply()).valueOf(), "0");
      assert.equal((await this.sel.balanceOf(bob)).valueOf(), "0");
      assert.equal((await this.sel.balanceOf(dev)).valueOf(), "0");
      assert.equal((await this.lp.balanceOf(bob)).valueOf(), "990");
      await time.advanceBlockTo("219");
      await this.master.withdraw(0, "10", { from: bob }); // block 220
      assert.equal((await this.sel.totalSupply()).valueOf(), "11000");
      assert.equal((await this.sel.balanceOf(bob)).valueOf(), "10000");
      assert.equal((await this.sel.balanceOf(dev)).valueOf(), "1000");
      assert.equal((await this.lp.balanceOf(bob)).valueOf(), "1000");
    });
    ...
    it("should distribute SUSHIs properly for each staker", async () => {
      ...
      await this.master.deposit(0, "10", { from: alice });
      assert.equal((await this.sel.totalSupply()).valueOf(), "11000");
      assert.equal((await this.sel.balanceOf(alice)).valueOf(), "5666");
      assert.equal((await this.sel.balanceOf(bob)).valueOf(), "0");
      assert.equal((await this.sel.balanceOf(carol)).valueOf(), "0");
      assert.equal(
        (await this.sel.balanceOf(this.master.address)).valueOf(),
        "4334"
      );
      assert.equal((await this.sel.balanceOf(dev)).valueOf(), "1000");
      ...
      assert.equal((await this.sel.totalSupply()).valueOf(), "22000");
      assert.equal((await this.sel.balanceOf(alice)).valueOf(), "5666");
      assert.equal((await this.sel.balanceOf(bob)).valueOf(), "6190");
      assert.equal((await this.sel.balanceOf(carol)).valueOf(), "0");
      assert.equal(
        (await this.sel.balanceOf(this.master.address)).valueOf(),
        "8144"
      );
      assert.equal((await this.sel.balanceOf(dev)).valueOf(), "2000");
      ...
      assert.equal((await this.sel.totalSupply()).valueOf(), "55000");
      assert.equal((await this.sel.balanceOf(dev)).valueOf(), "5000");
      // Alice should have: 5666 + 10*2/7*1000 + 10*2/6.5*1000 = 11600
      assert.equal((await this.sel.balanceOf(alice)).valueOf(), "11600");
      // Bob should have: 6190 + 10*1.5/6.5 * 1000 + 10*1.5/4.5*1000 = 11831
      assert.equal((await this.sel.balanceOf(bob)).valueOf(), "11831");
      // Carol should have: 2*3/6*1000 + 10*3/7*1000 + 10*3/6.5*1000 + 10*3/4.5*1000 + 10*1000 = 26568
      assert.equal((await this.sel.balanceOf(carol)).valueOf(), "26568");
    ...
    it("should give proper SUSHIs allocation to each pool", async () => {
      ...
      assert.equal(
        (await this.master.pendingSushi(0, alice)).valueOf(),
        "10000"
      );
      ...
      assert.equal((await this.master.pendingSushi(1, bob)).valueOf(), "3333");
    });
    ...
    it("should stop giving bonus SUSHIs after the bonus period ends", async () => {
      ...
      assert.equal((await this.master.pendingSushi(0, alice)).valueOf(), "0");
      assert.equal((await this.sel.balanceOf(alice)).valueOf(), "10600");
    });
  });
});

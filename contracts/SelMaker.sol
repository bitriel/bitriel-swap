...
// SELMaker is MasterChef's left hand and kinda a wizard. He can cook up SEL from pretty much anything!
//
...
contract SelMaker {
    ...
    constructor(IUniswapV2Factory _factory, address _bar, address _sel, address _weth) public {
        factory = _factory;
        sel = _sel;
        bar = _bar;
        weth = _weth;
    ...
    function _toWETH(address token) internal returns (uint256) {
        ...
        if (token == weth) {
            uint amount = IERC20(token).balanceOf(address(this));
            _safeTransfer(token, factory.getPair(weth, sel), amount);
            return amount;
        }
        ...
        pair.swap(amount0Out, amount1Out, factory.getPair(weth, sel), new bytes(0));
        return amountOut;
    }

    // Converts WETH to SEL
    function _toSUSHI(uint256 amountIn) internal {
        IUniswapV2Pair pair = IUniswapV2Pair(factory.getPair(weth, sel));
